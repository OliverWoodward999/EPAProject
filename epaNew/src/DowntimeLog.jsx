...
// Use env var if provided (e.g. VITE_API_BASE=http://localhost:5001 during dev)
// Otherwise default to same-origin (''), so Nginx proxies /api -> Node.
const API_BASE =
...
function DowntimeLog() {
  ...
  const fetchDowntimeEntries = async () => {
    try {
      const res = await fetch(apiUrl(`/api/downtime?username=${encodeURIComponent(username)}`));
